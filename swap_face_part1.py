...
def main(swap, image, video):
    if swap:
        ...
        while(success):
            ...
            _, image2, srcPoints, faceDetected = getTri(imageSource, True)
            ...
            subdivDest,image1,destPoints,faceDetected = getTri(imageDest, False)
            ...
            if(faceDetected):
                ...
                if invSuccessFlag:
                    ...
                    cv2.imwrite('output_trian_1_' + str(i) + '.jpg', image1)
            invSuccessFlag = True
            if(faceDetected):
                # if firstRun:
                indexesTrianglesDest = getTriIndices(_,srcPoints,imageSource)
                    # firstRun = False
                # sourceFacesList = appendList(sourceFacesList, srcPoints)
                # srcPoints = avgList(sourceFacesList)
                ...
                # print(len(destPoints))
                mask_moments = np.zeros((imageSource.shape[0], imageSource.shape[1]))
                mask = np.zeros(imageSource.shape, imageSource.dtype)
                firstFlag = True
                for triangle_index in indexesTrianglesDest:
                    src_pt1 = destPoints[triangle_index[0]]
                    src_pt2 = destPoints[triangle_index[1]]
                    src_pt3 = destPoints[triangle_index[2]]
                    dst_pt1 = srcPoints[triangle_index[0]]
                    dst_pt2 = srcPoints[triangle_index[1]]
                    dst_pt3 = srcPoints[triangle_index[2]]
                    ...
                    if(invSuccessFlag):
                        replacePixels(imageSource, interpObjs, bInv, a, rect_dst, mask, mask_moments)
                ...
                if invSuccessFlag:
                    ...
                    image1 = cv2.seamlessClone(
                        imageDest, image1, mask, (cX, cY), cv2.NORMAL_CLONE)
                    cv2.imwrite('output_trian_2_' + str(i) + '.jpg', image1)
        ...
        while (success):
            imageDestPoisson = copy.deepcopy(imageDest)
            imageDestTri = copy.deepcopy(imageDest)
            subdivDest, image1, destPoints, faceDetected = getTri(imageDest, True)
